...
func TestUserHandlers_GetPRAuthorityStatistic(t *testing.T) {
	t.Parallel()
	...
	router := gin.New()
	router.GET("/users/getPRAuthorityStatistic", func(c *gin.Context) {
		impl.GetUsersGetPRAuthorityStatistic(c)
	})
	...
	t.Run("success", func(t *testing.T) {
		...
		userUC.EXPECT().
			UsersGetPRAuthorityStatistic(gomock.Any()).
			Return(expected, nil)

		w := httptest.NewRecorder()
		req := httptest.NewRequest(http.MethodGet, "/users/getPRAuthorityStatistic", nil)
